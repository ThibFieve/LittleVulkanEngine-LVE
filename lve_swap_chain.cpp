...
LveSwapChain::LveSwapChain(LveDevice& deviceRef, VkExtent2D extent, std::shared_ptr<LveSwapChain> previous)
    ...
    //cleam up old swap chain since it's no longer needed
...
void LveSwapChain::createSwapChain() {
  ...
  // Check if the oldswapchain is a null ptr, if yes return Null handle , otherhwise give the swapchain of Oldsapchain.
...
VkPresentModeKHR LveSwapChain::chooseSwapPresentMode(
  ...
  for (const auto &availablePresentMode : availablePresentModes) {
    if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) { // depending on our graphic device it is settupt to choose the mailbox present mode per default and Vsync as fallback
      std::cout << "Present mode: Mailbox" << std::endl; //  present mopde define how our swap chain handle  synchroniation  with the display
      return availablePresentMode; // by default it will be vsyn " FIFO" see explaination  https://www.youtube.com/watch?v=IUYH74MqxOA https://vulkan-tutorial.com/Drawing_a_triangle/Presentation/Swap_chain
    }
  }
   // to Alwasy use V-sync comment out those lines
  // for (const auto &availablePresentMode : availablePresentModes) {
