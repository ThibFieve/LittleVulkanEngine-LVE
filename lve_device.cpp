...
// Create Vulkan instance , Validation Layers, Physical device and queue family setup , logival device setup 
...
//LveDevice::LveDevice(LveWindow& window) : window{ window } {   // CONSTRUCTOR 
 ...
 //   createLogicalDevice(); // create logical device , what features of our physical device we want to use
  //  createCommandPool(); // convenient to create a command pool for later use , help for command buffer allocation later on 
...
void LveDevice::hasGflwRequiredInstanceExtensions() {
  ...
  std::cout << "available extensions:" << std::endl;               // THIS IS WHERE WE PRINT THE EXTENSION WE NEED AND THOPSE WE HAVE 
